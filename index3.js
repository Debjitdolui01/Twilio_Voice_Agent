...
STEP 4: Ask for preferred date
      ...
      -If user provides a natural language date (e.g., 'tomorrow', 'day after tomorrow', 'next Friday'), accept it and proceed.
      -If user provides invalid or past date, respond: "Please provide a valid future date like '26 October 2025'."
    ...
    fastify.get('/media-stream', { websocket: true }, (connection, req) => {
        ...
        const TOOLS = {
            ...
            book_appointment: async ({ name, date, purpose }) => {
                ...
                console.log({ name, date, purpose, phoneNumber: callerPhoneNumber });
                ...
                // if (appointmentDate <= today) {
                //     return "Appointment date must be in the future. Please provide a future date.";
                // }

              // Parse natural language date using chrono (supports "tomorrow", "next monday", etc.)
                    let appointmentDate = chrono.parseDate(date, new Date(), { forwardDate: true });
                    if (!appointmentDate) {
                    ...
                    console.log('Is parsed date <= today?', appointmentDate <= today);

                    // If the parsed date is in the past, try parsing again with more context
                    if (appointmentDate <= today) {
                        // Try re-parsing with explicit future context
                        const newParse = chrono.parse(date, new Date());
                        if (newParse.length > 0) {
                            const newDate = newParse[0].start.date();
                            newDate.setHours(0, 0, 0, 0);
                            if (newDate > today) {
                                appointmentDate = newDate;
                                console.log('Re-parsed future date:', appointmentDate);
                            }
                        }
                    }

                    // Final validation to ensure future date
                    if (appointmentDate <= today) {
                ...
                if (storageResult.success) {
                    ...
                    return `Appointment successfully booked for ${name}! Date: ${date}, Purpose: ${purpose}. Your appointment is pending confirmation. ${callerPhoneNumber ? 'A confirmation message has been sent to your phone.' : 'Please note your appointment details.'}`;
                } else {
                    return `Appointment details recorded for ${name}, Date: ${date}, Purpose: ${purpose}. However, there was an issue storing the appointment in our system. Please contact us directly to confirm.`;
                }
            }
        };
